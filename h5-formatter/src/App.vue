...
const language = ref<Language>('html')

// 语言特定选项
const semiColons = ref(true) // JavaScript
const singleQuote = ref(false) // HTML/JavaScript
const bracketSameLine = ref(false) // HTML

const highlightedOutput = computed(() => {
  if (!output.value) return ''
  try {
    return hljs.highlight(output.value, { language: language.value }).value
  } catch {
    return output.value
  }
})
...
watch([input, indent, singleQuote, bracketSameLine, semiColons, language], async () => {
  if (!input.value.trim()) {
    output.value = ''
    return
  }
...
const placeholderText = computed(() => {
  switch (language.value) {
    case 'html':
      return '粘贴或输入 HTML 代码...'
    case 'css':
      return '粘贴或输入 CSS 代码...'
    case 'javascript':
      return '粘贴或输入 JavaScript 代码...'
    default:
      return '粘贴或输入代码...'
  }
})
...
<template>
  ...
  <div class="flex-1 flex flex-col min-h-0">
    ...
    >
      <select
        v-model="language"
        class="h-8 px-3 rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
      >
      ...
      <div class="ml-auto flex items-center gap-4">
        ...
        <Label
          v-if="language === 'html' || language === 'javascript' || language === 'css'"
          class="flex items-center gap-2 text-sm text-gray-600 font-normal cursor-pointer"
        >
        ...
        <Label
          ...
          class="flex items-center gap-2 text-sm text-gray-600 font-normal cursor-pointer"
        >
          <Checkbox v-model:checked="bracketSameLine" />
          标签同行
        </Label>
